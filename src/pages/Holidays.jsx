    ...
    <div className="min-h-screen bg-canvas text-ink flex flex-col font-inter">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-surface-lowest border border-white/8 text-white rounded-lg py-2.5 px-4 text-sm focus:outline-none focus:border-white/30 font-inter placeholder:text-neutral-600 transition-colors"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-surface-lowest border border-white/8 text-white rounded-lg py-2.5 px-3 text-sm focus:outline-none focus:border-white/30 font-inter font-mono transition-colors"
                  />
                </div>
              ...
              {totalDaysCalculated > 0 && (
                <div className="p-4 bg-surface-lowest border border-white/8 rounded-lg flex items-center justify-between text-xs font-mono">
                  <span className="text-muted flex items-center gap-1.5 font-geist uppercase font-bold tracking-wide text-[10px]">
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full bg-white hover:bg-white/90 text-canvas font-bold py-3 rounded-lg transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer font-geist uppercase tracking-wider text-xs"
              >
          ...
          {/* History Card */}
          <div className="lg:col-span-6 bg-surface-card/70 backdrop-blur-md border border-white/8 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.08)] rounded-xl p-6 shadow-xl flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto space-y-3 max-h-[450px] pr-1">
              {loading ? (
                <div className="text-center py-8 text-muted text-xs font-mono">Memuat riwayat libur...</div>
              ) : holidays.length === 0 ? (
                ...
                holidays.map((h) => (
                  <div
                    key={h.id}
                    className="p-4 bg-surface-lowest border border-white/8 rounded-lg flex items-center justify-between"
                  >
                    <div>
